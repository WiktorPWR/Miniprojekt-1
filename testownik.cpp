...
int main()
{
    ...
    nanoseconds array[4][ilosc_powtorzen];
    ...
    for (int i = 0; i < ilosc_powtorzen; i++)
    {
        ...
        //Dobra jest gotowy dynamic array
        //Testowanie push back i potem pop back
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < ilosc_powtorzen; j++) {
            srednie[i] += array[i][j].count(); // Dodajemy czas wykonania do sumy
        }
